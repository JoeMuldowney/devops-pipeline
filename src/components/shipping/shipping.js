...
const Shipping = () => {
  ...
  useEffect(() => {
    const getBilling = async () => {
      try {
        ...
        const resp = await axios.get('http://localhost:8888/cart/shipping', {
          params: { user: userId }
        });

        const response = await axios.get('http://localhost:8888/cart/allshipping', {
          params: { user: userId }
        });
  ...
  const setAddressClick = (id) => {
    axios
      .put('http://localhost:8888/cart/updateshipping', { withCredentials: true }, {
